...
<body>
	<div class="easyui-layout" fit="true" scroll="no">
		<div  data-options="region:'west',title:'组织结构定义',split:true" style="width:200px;overflow: auto;">
    ...
    <!-- <iframe width="100%" height="100%" id="center"  src="" style="border:1px #fff solid; background:#fff;"></iframe> -->
		...
		<div class="hidden">
			...
			<div id="gysMenuGW2" class="easyui-menu" data-options="onClick:menuHandler" style="width: 120px;">
				<div data-options="name:'editSupplier'">编辑</div>
				<div data-options="name:'removeSupplier'">删除 </div>
				<div data-options="name:'fresh'">刷新</div>
			</div>
...
function createFramett(id,url) {
	var s = '<iframe id="'+id+'" scrolling="no" frameborder="0" src="'+url+'" width="100%" height="100%" style="display:block"></iframe>';
	return s;
}
...
function loadTree() {
	var zNodes;
	var ztreeCreator = new ZtreeCreator('orgTree',"organzationController.do?getMyTreeData",zNodes)
	...
	setTimeout(function(){
		if(flag){
			...
			if(orgType=="1"){
				...
				//addtt('基本信息', url, '01','icon-comturn', 'false');
				url = "organzationController.do?myUserOrgList&departid="+treeNode.id;
				addtt('用户信息', url, '01','icon-user-set', 'false');
				/* url = "tSCompanyPositionController.do?list&companyId="+treeNode.id;
				addtt('职务信息', url, '03','icon-chart-organisation', 'false'); */
			}else if(orgType=="3"){
				//url = "organzationController.do?myUserOrgList&departid="+treeNode.id;
				...
				url = "organzationController.do?myUserOrgListSuborg&departid="+treeNode.id;
				addtt('用户信息', url, '01','icon-user-set', 'false');
			}else {
				url = "organzationController.do?myUserOrgListSuborg&departid="+treeNode.id;
				addtt('用户信息', url, '01','icon-user-set', 'false');
...
function addSubCompany() {
	...
	if(selectNode.isParent){
		tip('该部门存在下级部门，不可删除！');return;
	}
	$.messager.confirm("提示","确认删除该部门？",function(r){
		if(r){
			$.ajax({
				url: "myDepartController.do?departDel&id="+selectNode.id,
				type: "get",
...
/* 	layer.confirm('确定删除该机构及机构下所有账号吗？',{
		btn:['确认','取消']
	},function() {
		jQuery.ajax({
	        async : false,
