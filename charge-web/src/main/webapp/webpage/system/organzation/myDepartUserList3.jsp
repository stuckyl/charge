...
function within(title,url, id){
	var url = "myDepartController.do?changeDepartManager";
	var rowsData = $('#'+id).datagrid('getSelections');
	if (!rowsData || rowsData.length==0) {
		layer.alert('请选择一个用户');
		return;
...
<!-- <div id="divUserList" class="easyui-layout" style="width:100%;">
	...
	<t:datagrid name="departUserList" title="common.operation"
	            actionUrl="organzationController.do?userDatagrid&departid=${departid}" fit="true" fitColumns="true" idField="id" pageSize="20" queryMode="group" sortName="userName">
		...
		<%-- <t:dgToolBar title="权限更改" icon="icon-edit" url="#" funname="within"></t:dgToolBar> --%>
