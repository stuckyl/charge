...
/**
 * 动态显示 选中行的 统计信息
 * reporter用户能看到的合计项（收入、毛利、项目数、平均毛利、毛利率）
 */
...
function declareSubmit(){
	   ...
	   if(id == "") {
		...
		if(declares!="") {
			...
			var arr1 = id.toString().split(',');

			//var arr1 = index.toString().split(',');
			//var arr2 = names.toString().split(",");
			for(var i=0;i<arr.length;i++) {
				if(arr[i]!="${actionCode-1+2}"&arr[i]!="${actionCode+2}") {
					isErr=true;
					for(var j=0;j<index.length;j++){
						if(index[j].id==arr1[i]){
							var a = j+1;
							tip1+= a +",";
						}
					}
		...
		if(isErr) {
			layer.alert('编号：'+tip1+'的项目当前状态不可申报');
			return false;
		}
		//if(!(id=="")){
...
/* function backSubmit(){
	   ...
	   //还原标记
	    var backUpFlag = getprojectListSelections("backUpFlag");
	   //状态标记
	    var declares = getprojectListSelections("projectStatus");
	    var sessionUser = "${sessionScope.LOCAL_CLINET_USER.userName }";
	    var inputNames = getprojectListSelections("inputerId");
		...
		if(declares!="") {
			var arr = declares.toString().split(',');
			var arr1 = backUpFlag.toString().split(',');
			var arr2 = sessionUser.toString().split(',');
			var arr3 = inputNames.toString().split(',');
			//var arr2 = names.toString().split(",");
			for(var i=0;i<arr.length;i++) {
				if(arr[i]!=1&&arr1[i]==1) {
				...
				if(arr3[i]!=sessionUser){
					isErr2=true;
				}
			}
		}
		if(isErr) {
			layer.alert('当前项目状态不可还原');
			return false;
		}
		if(isErr2) {
		...
		$.ajax({
			url:"projectController.do?projectRestore&ids="+ids,
			type:"get",
			success:function(data){
				data = JSON.parse(data);
				if(data.errCode==0){
...
function returnSubmit(){
	   ...
	   if(id == "") {
		...
		if(isErr3) {
			layer.alert('编号：'+tip4+'的项目当前状态不可拒绝');
			return false;
		}
...
function newDelSubmit(){
	   ...
	   var index =  $('#projectList').datagrid('getRows');
		...
		if(declares!="") {
			var arr = declares.toString().split(',');
			var arr1 = id.toString().split(",");
			var arr2 = backUpFlag.toString().split(",");
			var arr3 = inpName.toString().split(",");
			for(var i=0;i<arr.length;i++) {
				if(!(arr[i] == "${actionCode+2}" ||arr[i]=="${actionCode-1+2}")) {
					isErr = true;
					for(var j=0;j<index.length;j++){
						if(index[j].id==arr1[i]){
							var a = j+1;
...
function excelExport(){
  ...
  <t:datagrid name="projectList" title="项目录入" pageSize="20"
 ...
 <!--  onDblClick="update('查看','employeeDeclareController.do?lookorcheck','employeeDeclareList','100%','100%')" -->
   ...
   <t:dgCol title="申报状态" field="projectStatus"   width="120" query="true" replace="审批通过_1,待处理_${actionCode+2},失败_${actionCode-1+2},未上报_${actionCode+1+2},已上报_${actionCode-2+2}," extendParams="resizable:false,styler: function(value,row,index){if(value=='${actionCode-1+2}'||value=='${actionCode+2}'){return 'color:red;';}}" align="center"></t:dgCol>
   ...
   <t:dgToolBar title="新增" icon="icon-add" width="100%" height="100%" url="projectController.do?addReporter" funname="add" ></t:dgToolBar>
...
<%--    <t:dgToolBar title="还原" icon="icon-le-reback" url="#" funname="backSubmit" ></t:dgToolBar> --%>
   <t:dgToolBar title="删除" url="#" icon="icon-remove" funname="newDelSubmit" ></t:dgToolBar>
