...
function returnSubmit(){
	...
	dcAlert("<div><p>拒绝理由：</p><textarea rows='10' id='returnReason' style='width: 300px;'>"+oldReason+"</textarea></div>",true,function(){
		...
		if(null==returnReason||""==$.trim(returnReason)){
			...
			$.ajax({
				url:"employeeDeclareController.do?batchReturn2&returnReason="+encodeURI(encodeURI(returnReason),'UTF-8')+"&type=1&ids="+ids,
				type:"get",
...
/* function excelAccountExport(){
	var ids = getemployeeDeclareListSelections("id");
	if(ids == "") {
		//未选中数据
		var salaryDateChoose = $("input[name='salaryDateChoose']").val();

		location.href="employeeDeclareController.do?exportAccountExcel&month="+salaryDateChoose+"&corporateId="
				+$("input[name='corporateId']").val();
	}else {
 ...
 <style>
  ...
  <t:datagrid name="employeeDeclareList" title="收支审核" pageSize="20"
  ...
  <%-- actionUrl="employeeDeclareController.do?verifyDatagrid" --%>
<!--   onDblClick="update('查看','employeeDeclareController.do?lookorcheck','employeeDeclareList','100%','100%')" -->
 	...
 	<%--   <t:dgCol title="状态" field="declareStatus" align="center" replace="录入中_0,录入完成_1,申报中_2,申报拒绝_3,审核中_4,审核拒绝_5,审批中_6,审批拒绝_7,审批通过_10" width="120"></t:dgCol> --%>
 	  <t:dgCol title="状态" field="declareStatus" align="center"  width="100" query="true" replace="待申报_${actionCode-1 },申报拒绝_${actionCode-2 },未上报_${actionCode+1 },已上报_${actionCode-3 },审批通过_2"
 	  extendParams="styler: function(value,row,index){if(value=='${actionCode-1 }'||value=='${actionCode-2 }'){return 'color:red;';}}"></t:dgCol>
   ...
   <t:dgToolBar title="通过" icon="icon-le-ok" url="#" funname="passSubmit3" id="passBtn"></t:dgToolBar>
   <t:dgToolBar title="拒绝" icon="icon-le-back" url="#" funname="returnSubmit"  id="returnBtn"></t:dgToolBar>
    <%-- <t:dgToolBar title="还原" url="#" icon="icon-back" funname="restore" ></t:dgToolBar> --%>
   ...
   <%-- <t:dgToolBar title="导出网银" url="#" icon="icon-put" funname="excelAccountExport" ></t:dgToolBar> --%>
