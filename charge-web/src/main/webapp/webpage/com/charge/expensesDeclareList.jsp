...
<t:base type="jquery,easyui,tools,DatePicker"></t:base>
  <t:datagrid name="expensesDeclareList" title="经费申请"
   ...
   <t:dgCol title="申报金额" field="money" extendParams="styler: function(value,row,index){return 'text-align:right;'}" formatterjs="formatTwoDecimal,salaryFormat"></t:dgCol>
   ...
   <%-- <t:dgCol title="部门" field="departmentid" replace="${depts }" align="center" width="60"></t:dgCol> --%>
   <t:dgCol title="申报状态" field="declareStatus" replace="待处理_1,审核拒绝_2,待审核_3,审批拒绝_4,待审批_5,审批通过_6,"
   extendParams="resizable:false,styler: function(value,row,index){if(value=='1'||value=='2'){return 'color:red;';}}"
   ...
   <t:dgToolBar title="申报" icon="icon-le-ok" url="#" funname="entryComplete"  width="120"></t:dgToolBar>
...
function update(title,url, id,width,height,isRestful) {
	...
	var declares = getexpensesDeclareListSelections("declareStatus");
	var themes = getexpensesDeclareListSelections("theme");
	var isErr = false;
	var isErr1 = false;
	var tip = "";
	var tip1 = "";
...
function delSubmit(){
	var ids = getexpensesDeclareListSelections("id");
	if(ids == "") {
		layer.alert("未选中数据");
		return false;
	}
	var themes = getexpensesDeclareListSelections("theme");
	var declares = getexpensesDeclareListSelections("declareStatus");
	/* var names = getexpensesDeclareListSelections("employeeInfo.name");
	var types = getexpensesDeclareListSelections("employeeInfo.employeeFlag"); */
	var isErr = false;
	var isErr2 = false;
	/* var isErr3 = false; */
	var tip = "";
	...
	if(declares!="") {
		var arr = declares.toString().split(',');
		var arr2 = themes.toString().split(",");
		var arr3 = ids.toString().split(",");
		/* var arr3 = types.toString().split(","); */
		for(var i=0;i<arr.length;i++) {
			if(arr[i] != "1"&&arr[i] != "2") {
				isErr = true;
				tip+=arr2[i]+"，";
...
function entryComplete(){
	if($(".datagrid-btable tr").length<1){
	   ...
	   if(ids == "") {
		...
		if(isErr) {
			layer.alert("主题为： "+tip1+"的经费当前状态无法申报");
			return false;
		}
		//if(!(id=="")){
