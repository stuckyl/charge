...
function returnSubmit(){
	   ...
	   if(id == "") {
		...
		if(isErr) {
			layer.alert('当前项目状态不可拒绝');
			return false;
		}
		if(isErr3) {
			layer.alert('录入者为本人的项目无法拒绝');
			return false;
		}
...
function newDelSubmit(){
	   ...
	   if(id == "") {
		...
		if(isErr) {
			layer.alert('当前项目状态不可删除');
			return false;
		}
		if(isErr3) {
			layer.alert('录入者非本人的项目不可删除');
			return false;
		}
 ...
 */
 function OneKeyTotal() {
		...
		$.ajax({
			url:"projectController.do?oneKeyTotalByVisitor&month="+salaryDateChoose
	...
	function excelExport(){
		...
		if(ids == "") {
			//未选中数据
			location.href="projectController.do?excelExportforAccess&month="+salaryDateChoose
				+"&employeeDepartment="+$("select[name='projectDepartment']").val()
				+"&customerName="+$("select[name='projectCustomer1']").val()
				+"&declareStatus="+$("select[name='projectStatus']").val()
				+"&isAll="+$("select[name='isAll']").val();
		}else {
			location.href="projectController.do?excelExportSelectedIdsforAccess&month="+salaryDateChoose+"&ids="+ids;
		}

	}
</script>
  <t:datagrid name="projectList" title="项目审批" pageSize="20"
  actionUrl="projectController.do?projectFindbyMonth3"
  queryMode="group" fit="true" checkbox="true"
 ...
 <!--  onDblClick="update('查看','employeeDeclareController.do?lookorcheck','employeeDeclareList','100%','100%')" -->
     ...
     <t:dgCol title="原编号" field="batId" hidden="true"></t:dgCol>
   ...
   <t:dgToolBar title="查看" icon="icon-search" url="projectController.do?addReporterforAccess" funname="detail" height="100%" ></t:dgToolBar>
        ...
        onShowPanel: function () {
            ...
            if (!tds) setTimeout(function () {
                ...
                tds.click(function (e) {
                    ...
                    var salaryDateChoose = $("input[name='salaryDateChoose']").val();
                    $('#projectList').datagrid({
            			url:'projectController.do?projectFindbyMonth3&month='+salaryDateChoose,
            			pageNumber:1
            		});
