...
function declareSubmit(){
	...
	var names = getemployeeDeclareListSelections("employeeInfo.name");
	var inputNames = getemployeeDeclareListSelections("inputerId");
	var types = getemployeeDeclareListSelections("employeeType");
	var isErr = false;
	var tip = "";
	if(declares!="") {
		var arr = declares.toString().split(',');
		var arr2 = names.toString().split(",");
		var arr3 = inputNames.toString().split(",");
		var arr4 = types.toString().split(",");
		for(var i=0;i<arr.length;i++) {
			//可以申报
			if(arr[i]=="${actionCode-2}"||arr[i]=="${actionCode-1}") {
			}else {
				isErr = true;
				tip+=arr2[i]+"，";
			}
...
function returnSubmit(){
	...
	if(declares!="") {
		...
		for(var i=0;i<arr.length;i++) {
			//可以拒绝
			if((arr[i]=="${actionCode-1}"&&arr3[i]!=sessionUser)||(arr[i]=="${actionCode-2}"&&arr3[i]!=sessionUser)) {
			}else {
				isErr = true;
				tip+=arr2[i]+"，";
			}
	...
	dcAlert("<div><p>拒绝理由：</p><textarea rows='10' id='returnReason' style='width: 300px;'>"+oldReason+"</textarea></div>",true,function(){
		...
		if(null==returnReason||""==$.trim(returnReason)){
			...
			$.ajax({
				url:"employeeDeclareController.do?batchReturn2&returnReason="+encodeURI(encodeURI(returnReason),'UTF-8')+"&type=0&ids="+ids,
				type:"get",
				success:function(data){
					data = JSON.parse(data);
					if(data.errCode==0){
...
function delSubmit(){
	...
	if(declares!="") {
		...
		var arr2 = names.toString().split(",");
		var arr3 = inputNames.toString().split(",");
		var arr4 = baceUpFlags.toString().split(",");
		for(var i=0;i<arr.length;i++) {
			//可以删除
			if(arr4[i]!=1&&arr3[i]==sessionUser&&(arr[i]=="${actionCode}"||arr[i]=="${actionCode-1}"||arr[i]=="${actionCode-2}")) {
				//可以删除
			}else {
				isErr = true;
				tip+=arr2[i]+"，";
...
/* function restore() {
 ...
 <style>
  ...
  <t:datagrid name="employeeDeclareList" title="收支申报" pageSize="20"
  ...
  sortName="declareStatus" sortOrder="desc" >
      ...
      <t:dgCol title="收入" field="income" formatterjs="formatTwoDecimal,salaryFormat" extendParams="styler: function(value,row,index){return 'text-align:right;'}"  width="100"></t:dgCol>
      <t:dgCol title="毛利" field="companyProfit" formatterjs="formatTwoDecimal,salaryFormat"  extendParams="styler: function(value,row,index){return 'text-align:right;'}"  width="100"></t:dgCol>
      <t:dgCol title="毛利率" field="companyProfitRate" formatterjs="formatTwoDecimal,salaryFormat,companyProfitRateStyle"   extendParams="styler: function(value,row,index){return 'text-align:center;'}" width="100"></t:dgCol>
 	  <%-- <t:dgCol title="状态" field="declareStatus" align="center" replace="待录入_0,未申报_1,待通过_2,申报拒绝_3,审核中_4,审核拒绝_5,审批中_6,审批拒绝_7,审批通过_10" width="100" query="true"
 	  extendParams="styler: function(value,row,index){if(value=='2'||(value=='0'&&row.inputerId=='${sessionScope.LOCAL_CLINET_USER.userName }')||(value=='1'&&row.inputerId=='${sessionScope.LOCAL_CLINET_USER.userName }')||value=='5'||(row.employeeType==1&&value=='7')){return 'color:red;';}}"></t:dgCol> --%>
   	  <t:dgCol title="状态" field="declareStatus" align="center" width="100" query="true" replace="待录入_${actionCode },待申报_${actionCode-1 },申报拒绝_${actionCode-2 },未上报_${actionCode+1 },已上报_${actionCode-3 },审批通过_2"
   	  ...
   	  <t:dgCol title="失败理由" field="declareReturnreason" align="center" width="200" showLen="12"></t:dgCol>
   	   <t:dgCol title="录入者Id" field="inputerId" align="center" hidden="true"></t:dgCol>
   ...
   <t:dgToolBar title="申报" icon="icon-le-ok" url="#" funname="declareSubmit" ></t:dgToolBar>
  ...
  <%--  <t:dgToolBar title="还原" url="#" icon="icon-le-reback" funname="restore" ></t:dgToolBar> --%>
   ...
   <%-- <t:dgToolBar title="删除" url="#" funname="delSubmit" operationCode="delBtn"></t:dgToolBar> --%>
  </t:datagrid>
  <div>
<!--  -->
