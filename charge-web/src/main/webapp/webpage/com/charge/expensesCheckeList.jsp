...
<t:base type="jquery,easyui,tools,DatePicker"></t:base>
  <t:datagrid name="expensesCheckeList" title="经费审核"
   ...
   <t:dgCol title="部门" field="departmentId" replace="${depts }" query="true" align="center" width="60"></t:dgCol>
   <t:dgCol title="申报状态" field="declareStatus" replace="待处理_1,审核拒绝_2,待审核_3,审批拒绝_4,待审批_5,审批通过_6,"
   extendParams="resizable:false,styler: function(value,row,index){if(value=='3'||value=='4'){return 'color:red;';}}"
...
function update(title,url, id,width,height,isRestful) {
	...
	var declares = getexpensesCheckeListSelections("declareStatus");
	var themes = getexpensesCheckeListSelections("theme");
	var sessionUser = "${sessionScope.LOCAL_CLINET_USER.userName }";
	var createdBy = getexpensesCheckeListSelections("createdBy");
	var isErr = false;
	var isErr1 = false;
	var tip = "";
	var tip1 = "";
	if(declares!="") {
		...
		for(var i=0;i<arr.length;i++) {
			//可以编辑
			if(!((arr[i]==3&&arr3[i]==sessionUser)||(arr[i]==4&&arr3[i]==sessionUser))){
				isErr=true;
				tip +=arr2[i]+"，";
...
function delSubmit(){
	var ids = getexpensesCheckeListSelections("id");
	if(ids == "") {
		layer.alert("未选中数据");
		return false;
	}
...
function entryComplete(){
	if($(".datagrid-btable tr").length<1){
	   ...
	   if(ids == "") {
		...
		if(isErr) {
			layer.alert("主题为 "+tip1+"的经费当前状态无法审核通过");
			return false;
		}
		//if(!(id=="")){
...
function returnSubmit(){
	...
	var sessionUser = "${sessionScope.LOCAL_CLINET_USER.userName }";
	var createdBy = getexpensesCheckeListSelections("createdBy");
	var oldReason = getexpensesCheckeListSelections("declareReturnReason");
	var isErr = false;
	var isErr1 = false;
	var tip = "";
	var tip1 = "";
	if(declares!="") {
		...
		for(var i=0;i<arr.length;i++) {
			//可以退回
			if(!(arr[i]==3||(arr[i]==4&&arr3[i]!=sessionUser))){
				isErr=true;
				tip +=arr2[i]+"，";
			}/* else if(arr[i]==6){
				isErr1=true;
	...
	if(isErr) {
		layer.alert("主题为 "+tip+"的经费当前状态无法审核拒绝");
		return false;
	}
	dcAlert("<div><p>拒绝理由：</p><textarea rows='10' id='returnReason' style='width: 300px;'>"+oldReason+"</textarea></div>",true,function(){
		...
		if(null==returnReason||""==$.trim(returnReason)){
			...
			$.ajax({
				url:"expensesController.do?batchReturn2&returnReason="+encodeURI(encodeURI(returnReason),'UTF-8')+"&type=1&ids="+ids,
				type:"get",
				success:function(data){
					data = JSON.parse(data);
					if(data.errCode==0){
