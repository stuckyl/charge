...
function delSubmit(){
	...
	if(declares!="") {
		...
		for(var i=0;i<arr.length;i++) {
			if(arr[i]=="${actionCode}"||arr[i]=="${actionCode-1}"||arr[i]=="${actionCode-2}") {
			}else{
				isErr = true;
				tip+=arr2[i]+"，";
			}
...
<%--       <t:dgCol title="员工类型" field="employeeType" align="center" replace="TECH_0,OP_1" query="true" ></t:dgCol> --%>
      <t:dgCol title="收入" field="income"  formatterjs="formatTwoDecimal,salaryFormat" width="100" extendParams="styler: function(value,row,index){return 'text-align:right;'}"></t:dgCol>
      <t:dgCol title="毛利" field="companyProfit"  formatterjs="formatTwoDecimal,salaryFormat" width="100" extendParams="styler: function(value,row,index){return 'text-align:right;'}"></t:dgCol>
      <t:dgCol title="毛利率" field="companyProfitRate"  formatterjs="formatTwoDecimal,salaryFormat,companyProfitRateStyle" width="100" extendParams="styler: function(value,row,index){return 'text-align:center;'}"></t:dgCol>
