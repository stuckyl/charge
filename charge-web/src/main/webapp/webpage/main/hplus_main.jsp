...
<head>
    ...
    <meta name="keywords" content="收支运营 SaaS">
    <meta name="description" content="收支运营 SaaS">
...
<div id="wrapper">
        ...
        <div class="sidebar-collapse">
            <ul class="nav" id="side-menu">
                <li class="nav-header">
                    ...
                    <div class="logo-element">${roleName }</div>
                </li>
    ...
    <div id="page-wrapper" class="gray-bg dashbard-1">
        <div class="row border-bottom" id="div_a">
                ...
                <div class="navbar-header" style="height: 60px;"><a class="navbar-minimalize minimalize-styl-2 btn btn-primary " href="#"><i class="fa fa-bars"></i> </a>
                    ...
                    <form role="search" class="navbar-form-custom" method="post" action="search_results.html">
                        <div class="form-group">
                            <input type="text" placeholder="欢迎使用人事系统" class="form-control" name="top-search" id="top-search">
                        </div>
                    </form>
                     -->
                </div>
                ...
                <ul class="nav navbar-top-links navbar-right">
                   <%--  <li class="dropdown">
                    ...
                    <li class="dropdown" onfocus="bindFrameClick()">
                    	<a class="dropdown-toggle count-info" data-toggle="dropdown" href="#">
                                <span ><strong class="font-bold">${sessionScope.LOCAL_CLINET_USER.realName  }</strong></span>
                        ...
                        <ul class="dropdown-menu dropdown-alerts">
                            ...
                            <li><a href="javascript:add('<t:mutiLang langKey="common.profile"/>','userController.do?userinfo','',550,200)"><t:mutiLang langKey="common.profile"/></a></li>
                            <%-- <li><a href="javascript:openwindow('<t:mutiLang langKey="common.ssms.getSysInfos"/>','tSSmsController.do?getSysInfos')"><t:mutiLang langKey="common.ssms.getSysInfos"/></a></li> --%>
                            <!--<li><a href="javascript:add('<t:mutiLang langKey="common.change.style"/>','userController.do?changestyle','',550,250)"><t:mutiLang langKey="common.my.style"/></a></li> -->
                            <li><a href="javascript:clearLocalstorage()"><t:mutiLang langKey="common.clear.localstorage"/></a></li>
                            <!-- <li><a href="http://yun.jeecg.org" target="_blank">云应用中心</li> -->
                    ...
                    <li class="dropdown hidden-xs">
                      ...
                      -->
                      <li class="dropdown">
                     <a href="javascript:logout()" class="roll-nav roll-right J_tabExit" style="height:59.09px"><i class="fa fa fa-sign-out"></i> 退出</a>
                     </li>
                </ul>
            </nav>
        </div>
        <div class="row content-tabs" id="div_b">
            <button class="roll-nav roll-left J_tabLeft"><i class="fa fa-backward"></i>
            </button>
            <nav class="page-tabs J_menuTabs">
                <div class="page-tabs-content">
                    <a href="javascript:;" class="J_menuTab active" data-id="loginController.do?hplushome">首页</a>
                </div>
        ...
        <div class="row J_mainContent" id="content-main" style="margin-left:-13px;" id="content_main">
            <iframe class="J_iframe" name="iframe14" width="100%" height="100%" src="loginController.do?hplushome" frameborder="0" data-id="loginController.do?hplushome" seamless="" style="display: inline;"></iframe></iframe>
        </div>
       <%--  <div class="footer">
            <div class="pull-right">&copy; <t:mutiLang langKey="system.version.number"/></div>
        </div> --%>
    </div>
    <!--右侧部分结束-->
    <!--右侧边栏开始-->
...
<script>
    ...
    function sizeAuto(){
    	 //设置div自动适应
    	var content_main=document.getElementById("content-main");//取得id为content_main的div标签
        var all=document.getElementById("wrapper");
        var a=document.getElementById("div_a");
        var b=document.getElementById("div_b");
        content_main.style.height=((all.clientHeight)-(a.clientHeight)-(b.clientHeight)-3)+"px";//all.clientHeight取得all的长度
    }
    var methodN=0;
    $(document).ready(function(){
    	...
    	getMyRoleNum();
    	 if(methodN==1) {
    		getSystemMessage1();
    		setInterval(getSystemMessage1,5000);
    	}else if(methodN==2) {
    		getSystemMessage2();
    		setInterval(getSystemMessage2,5000);
    	}else if(methodN==3) {
    		getSystemMessage3();
    	    setInterval(getSystemMessage3,5000);
    	}else if(methodN==4) {
    		getSystemMessage4();
    	    setInterval(getSystemMessage4,5000);
    	}
    });
    function getMyRoleNum() {
    	$.ajax({
    		url:"employeeDeclareController.do?getMyRoleNum",
    		type:"GET",
    		async:false,
    ...
    //客户经理
    function getSystemMessage1() {
    	/* $.ajax({
			url:"employeeDeclareController.do?getMyMessageCout",
			type:"GET",
			async:false,
			success:function(data) {
				if(data==0) {
					$("a[href$='employeeDeclareController.do?entrylist']>span").eq(1).empty();
				}else{
					$("a[href$='employeeDeclareController.do?entrylist']>span").eq(1).remove();
					$("a[href$='employeeDeclareController.do?entrylist']>span").after("<span style='float:right;color:red'>"+data+"</span>");
				}
			}
    	});
    	$.ajax({
			url:"employeeInfoController.do?pendingData",
			type:"GET",
			async:false,
			success:function(data) {
				if(data==0) {
					$("a[href$='employeeInfoController.do?employeeInfoDeclareList']>span").eq(1).empty();
				}else{
					$("a[href$='employeeInfoController.do?employeeInfoDeclareList']>span").eq(1).remove();
					$("a[href$='employeeInfoController.do?employeeInfoDeclareList']>span").after("<span style='float:right;color:red'>"+data+"</span>");
				}
			}
    	}); */
    	getDeclareNumAjax(1,'employeeInfoController.do?employeeInfoDeclareList','employeeDeclareController.do?entrylist','projectController.do?projectInputerList');
    }
    ...
    function getSystemMessage2() {
    	//员工信息申报待处理数
    	//收支申报 1人力 2项目
    	getDeclareNumAjax(1,'employeeInfoController.do?employeeInfoCheckList','employeeDeclareController.do?declarelist','projectController.do?list');
    }
    //审计
    function getSystemMessage3() {
    	getDeclareNumAjax(5,'employeeInfoController.do?employeeInfoApprovalList','employeeDeclareController.do?verifylist','projectController.do?projectCheckerList');
    }
    //控制
    function getSystemMessage4() {
    	getDeclareNumAjax(6,'employeeInfoController.do?employeeInfoControl','employeeDeclareController.do?approvallist','projectController.do?projectControllerList');
    }
    /**
    *@param menuNum 收支申报菜单排序编号 $(".menu-text").eq(menuNum)
    *@param hrefemp 定位 员工信息申报菜单 href
    *@param hrefrl 定位 人力收支申报菜单 href
    *@param hrefxm 定位 项目收支申报菜单 href
    */
    function getDeclareNumAjax(menuNum,hrefemp,hrefrl,hrefxm) {
    	var pendingrl=0;
    	var pendingxm=0;
    	//员工
    	$.ajax({
    		...
    		success:function(data) {
    			if(data==0) {
    				$("a[href$='"+hrefemp+"']>span").eq(1).empty();
    			}else{
    				$("a[href$='"+hrefemp+"']>span").eq(1).remove();
    				$("a[href$='"+hrefemp+"']>span").after("<span style='float:right;color:red'>"+data+"</span>");
    			}
    		}
    	});
    	//人力
    	...
    	//项目
    	$.ajax({
    		url:"projectController.do?getMyMessageCout",
    		type:"GET",
    		async:false,
    		success:function(data) {
    			pendingxm=data;
    	...
    	if(pendingrl==0&&pendingxm==0){
    		...
    		//收支申报一级菜单 加 *
    		$("#dttx-szsb").remove();
    		$(".menu-text").eq(menuNum).after("<span style='float:right;color:red' id='dttx-szsb'>*</span>");
    		if(pendingrl!=0) {
    			$("#dttx-szsb-rl").remove();
    			$("a[href$='"+hrefrl+"']>span").after("<span style='float:right;color:red' id='dttx-szsb-rl'>"+pendingrl+"</span>");
    		}
    		if(pendingxm!=0) {
    			$("#dttx-szsb-xm").remove();
    			$("a[href$='"+hrefxm+"']>span").after("<span style='float:right;color:red' id='dttx-szsb-xm'>"+pendingxm+"</span>");
    		}
    	}
    }
