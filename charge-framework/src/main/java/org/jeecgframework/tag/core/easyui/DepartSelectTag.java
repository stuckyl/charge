...
public class DepartSelectTag extends TagSupport {
	...
	private boolean radioEnabled = false;        //是否单选
	...
	public StringBuffer end() {
		...
		if(hasLabel && oConvertUtils.isNotEmpty(title)){
			...
			sb.append("<span style='vertical-align:middle;display:-moz-inline-box;display:inline-block;width: 90px;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; ' title='"+title+"'>");
		...
		sb.append("    $.dialog({content: 'url:departController.do?departSelect" + orgIds + "&isRadio="+radioEnabled+"', zIndex: 2100, title: '" + commonDepartmentList + "', lock: true, width: '" + windowWidth + "', height: '" + windowHeight + "', opacity: 0.4, button: [");
