...
//			sb.append("},");
			...
			if (dialog) {
				if(callback!=null&&callback.contains("@Override")){//复写默认callback
					...
					sb.append("if(data.success==true){frameElement.api.close();win.layer.msg(data.msg);}else{if(data.responseText==''||data.responseText==undefined){win.layer.msg(data.msg);$.Hidemsg();}else{try{var emsg = data.responseText.substring(data.responseText.indexOf('错误描述'),data.responseText.indexOf('错误信息')); win.layer.msg(emsg);$.Hidemsg();}catch(ex){win.layer.msg(data.responseText+\"\");$.Hidemsg();}} return false;}");
					//
					if (refresh) {
