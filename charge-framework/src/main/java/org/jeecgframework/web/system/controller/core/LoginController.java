...
public class LoginController extends BaseController{
	...
	@ResponseBody
	public AjaxJson checkuser(TSUser user, HttpServletRequest req,HttpServletResponse res) {
		...
		if(remember!=null&&"0".equals(remember)) {
			Cookie cookie = new Cookie("JEECGUSERNAME", user.getUserName());
			//设置cookie有效期为一个月
		...
		int flag = 0;
		if (u == null) {
			List<TSUser> sysPhoneUser = userService.findByProperty(TSUser.class, "mobilePhone", user.getUserName());
			List<TSUser> loginPhoneUser = new LinkedList<TSUser>();
			if(sysPhoneUser.size()>0){  // 多于一个账号 用输入手机号
				for(int i=0;i<sysPhoneUser.size();i++){
					if(sysPhoneUser.get(i).getPassword().equals(PasswordUtil.encrypt(sysPhoneUser.get(i).getUserName(), user.getPassword(), PasswordUtil.getStaticSalt()))){
						flag = 1;
						if(sysPhoneUser.get(i).getStatus()!=0){   //未锁定
							loginPhoneUser.add(sysPhoneUser.get(i));
						}
					}
				}
				if(loginPhoneUser.size()>0){    //正确用户 的所有账号
					if(loginPhoneUser.size()==1) {
						...
						if (oConvertUtils.isEmpty(orgId)) {
							...
							Long orgNum = systemService.getCountForJdbc("select count(1) from t_s_user_org where user_id = '" + loginPhoneUser.get(0).getId() + "'");
							if (orgNum > 1) {
								attrMap.put("orgNum", orgNum);
								attrMap.put("user", loginPhoneUser);
							} else {
								Map<String, Object> userOrgMap = systemService.findOneForJdbc("select org_id as orgId from t_s_user_org where user_id=?", loginPhoneUser.get(0).getId());
								saveLoginSuccessInfo(req,res, loginPhoneUser.get(0), (String) userOrgMap.get("orgId"));
							}
						} else {
							attrMap.put("orgNum", 1);
							saveLoginSuccessInfo(req,res, u, orgId);
						}
					}else {// 处理用户有多个组织机构的情况，以弹出框的形式让用户选择
						Map<String, Object> attrMap = new HashMap<String, Object>();
						j.setAttributes(attrMap);
						attrMap.put("orgNum", loginPhoneUser.size());
						attrMap.put("user", loginPhoneUser);
					}
				}else if(loginPhoneUser.size()==0&&flag==1){  //只有一个却被锁定
			...
			if (oConvertUtils.isEmpty(orgId)) {
				...
				if (orgNum > 1) {
					...
					saveLoginSuccessInfo(req,res, u, (String) userOrgMap.get("orgId"));
				}
			} else {
				attrMap.put("orgNum", 1);
				saveLoginSuccessInfo(req,res, u, orgId);
	...
	@ResponseBody
	public AjaxJson changeDefaultOrg(TSUser user, HttpServletRequest req,HttpServletResponse res) {
		AjaxJson j = new AjaxJson();
		Map<String, Object> attrMap = new HashMap<String, Object>();
		String orgId = req.getParameter("orgId");
		TSUser u = userService.checkUserExits(user);
     ...
     * @param orgId 组织主键
     */
    private void saveLoginSuccessInfo(HttpServletRequest req,HttpServletResponse res, TSUser user, String orgId) {
    	String message = null;
	...
	public String login(ModelMap modelMap,HttpServletRequest request,HttpServletResponse response) {
		...
		if (user != null) {
            ...
            modelMap.put("roleName", roles.length()>15?roles.substring(0,10)+"...":roles);
            modelMap.put("userName", user.getUserName().length()>15?user.getUserName().substring(0, 10)+"...":user.getUserName());
            modelMap.put("realName", user.getRealName().length()>15?user.getRealName().substring(0, 10)+"...":user.getRealName());
