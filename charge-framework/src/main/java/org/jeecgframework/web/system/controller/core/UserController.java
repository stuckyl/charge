...
**/
	...
	@RequestMapping(params = "changeUserInfo")
	@ResponseBody
	public AjaxJson changeUserInfo(HttpServletRequest req, TSUser user) {
		AjaxJson j = new AjaxJson();
		TSUser users = systemService.getEntity(TSUser.class, user.getId());
		users.setRealName(user.getRealName());
		users.setEmail(user.getEmail());
		users.setMobilePhone(user.getMobilePhone());
		req.getSession().setAttribute(ResourceUtil.LOCAL_CLINET_USER, users);
		this.userService.saveOrUpdate(users);
		j.setMsg("信息更新成功");
		return j;
	}
	/**
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		List<TSRole> tsRole = new ArrayList<TSRole>();
		String[] roleid = null;
		if(orgid.equals("2c902db662f164650162f1ce49760041")){
			//控制权限
			tsRole = systemService.findByProperty(TSRole.class,"roleCode",(Object)"t_control");
			roleid = new String[]{tsRole.get(0).getId()};
		}else if (orgid.equals("2c902db662f164650162f1ce6b5a0043")) {
			//审核权限
			tsRole = systemService.findByProperty(TSRole.class,"roleCode",(Object)"t_check");
			roleid = new String[]{tsRole.get(0).getId()};
		}else if (orgid.equals("2c902db662f164650162f1ce9ca40049")) {
			//访客
			tsRole = systemService.findByProperty(TSRole.class,"roleCode",(Object)"t_access");
			roleid = new String[]{tsRole.get(0).getId()};
		}else{
				tsRole = systemService.findByProperty(TSRole.class,"roleCode",roleName);
				if(null!=tsRole&&0!=tsRole.size()){
					roleid = new String[]{tsRole.get(0).getId()};
				}
		}
		if (StringUtil.isNotEmpty(user.getId())) {
			TSUser users = systemService.getEntity(TSUser.class, user.getId());
			//密码加密
			String encryptPassword = PasswordUtil.encrypt(user.getUserName(), user.getPassword(), PasswordUtil.getStaticSalt());
			...
			users.setStatus(Globals.User_Normal);
			user.setDepartid(orgid);                          //设置depatrid
			users.setActivitiSync(user.getActivitiSync());
			//this.userService.saveOrUpdate(users, orgid.split(","), roleid.split(","));
			if (!(null == tsRole || tsRole.size() ==0)){
				this.userService.saveOrUpdate(users, orgid.split(" "), roleid);
			}else{
				this.userService.saveOrUpdate(users);
			}
			...
			if (users != null) {
				...
				this.userService.saveOrUpdate(user, orgid.split(" "), roleid);
...
//                saveUserOrgList(req, user);
	...
	public ModelAndView userOrgSelect(HttpServletRequest request) {
		...
		for(int i=0;i<=userIds.length-1;i++){
			...
			TSBaseUser user = systemService.getEntity(TSBaseUser.class, userIds[i]);
			rs.put("userName", user.getUserName());
			rs.put("realName", user.getRealName());
			rs1.put("user"+i,rs);
		}


        request.setAttribute("userList", rs1);
        /*try {
        	Thread.sleep(400); //1000 毫秒，也就是1秒.
...
//			}
	 ...
	 * @param user
	 */
	@RequestMapping(params = "addorupdateMyOrgUser2")
	public ModelAndView addorupdateMyOrgUser2(TSUser user, HttpServletRequest req) {
	    List<String> orgIdList = new ArrayList<String>();
	    TSDepart tsDepart = new TSDepart();
	    String departid = oConvertUtils.getString(req.getParameter("departid"));
	    if("2c902db662f164650162f1ce6b5a0043".equals(user.getDepartid())){
	    	return addorupdateMyOrgUser(user,req);
	    }
		if (StringUtil.isNotEmpty(user.getId())) {
			user = systemService.getEntity(TSUser.class, user.getId());
			List<TSRoleUser> tsRoleList =systemService.findHql("from TSRoleUser t where t.TSUser.id=?", new Object[]{user.getId()});
			if(!(null == tsRoleList || tsRoleList.size() ==0)){
				req.setAttribute("roleCode", tsRoleList.get(0).getTSRole().getRoleCode());
			}
			req.setAttribute("user", user);
			//String password = PasswordUtil.decrypt(user.getPassword(), password1, PasswordUtil.getStaticSalt());
		...
		req.setAttribute("tsDepart", tsDepart);
	    return new ModelAndView("system/user/myOrgUser2");
	}
	 ...
	 * @param response
	 */
	@RequestMapping(params = "roleNameList")
	@ResponseBody
	public List<String> roleNameList(TSUser user, HttpServletRequest req) {
		String departid = oConvertUtils.getString(req.getParameter("departid"));
		List<String> roleName =new ArrayList<String>();
		TSDepart org = systemService.getEntity(TSDepart.class,departid);
		if (org.getTSPDepart().getId().equals("2c902db662f164650162f1ce7e340045")) {
			TSRole role = systemService.getEntity(TSRole.class,"2c934f2663b556670163ba5184440176");
			roleName.add(role.getRoleName());
			roleName.add(role.getRoleCode());
		} else {
			String rolename = "t_report";
			List<TSRoleUser> roleUsers = systemService.findHql("from TSRoleUser t where t.TSUser.departid = ? and t.TSRole.roleCode = ? ",departid, rolename);
			if(roleUsers.size()>0){
				TSRole role = systemService.getEntity(TSRole.class,"2c934f26633da93b01633dac749f0003");
				roleName.add(role.getRoleName());
				roleName.add(role.getRoleCode());
			}else{
				TSRole role = systemService.getEntity(TSRole.class,"2c934f26633da93b01633dac749f0003");
				roleName.add(role.getRoleName());
				roleName.add(role.getRoleCode());
				TSRole role1 = systemService.getEntity(TSRole.class,"2c934f26633da93b01633dace53b0005");
				roleName.add(role1.getRoleName());
				roleName.add(role1.getRoleCode());
			}
		}
		return roleName;
	}
